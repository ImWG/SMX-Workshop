SMX Workshop Beta 1.9.2 帮助文档

关于
====================================================
该工具可用于读取、制作、修改《帝国时代》《帝国时代2》各版本的图形资源文件，包括《帝国时代》《帝国时代2》使用的SLP、《帝国时代：决定版》的SLP以及《帝国时代2：决定版》的SMP、SMX、SLD文件。通过程序，可以实现不同格式之间的转换，从图片文件导入或者将图形导出为图片，以及使用多种内部修改功能。

打开/保存文件
====================================================
除了通过菜单或者按快捷键Ctrl+O，您也可以通过将文件拖入程序窗口的方式来进行打开的操作。
目前支持读取的文件类型包括：SLP 2.0N版（《帝国时代》《帝国时代2》使用）、SLP 4.0X版（《帝国时代：决定版》曾使用）、SMP以及SMX（均为《帝国时代2：决定版》曾使用）、SLD（《帝国时代：决定版》与《帝国时代2：决定版》现使用）。不过，现在尚不支持将文件保存为SLP 4.0X或者SMP格式，且SLD格式读取时有所减少。
...
SMX——同SMP，但文件更小。
SLD——含有65536种普通色、256级玩家色浓度（覆盖于普通色上）、256级阴影。主体和阴影可以相互重叠。可选择附带65536种颜色的损伤图像（此工具暂不支持损伤）。是有损压缩。
...
损伤——单位受损时在主体图像上使用的不同等级的加深图像，生命值越低则加深程度越重。在SMP、SMX中，该数据实际上是和主体合并的。而SLD的损伤是RGB通道，三个通道的值决定了图像在单位轻、中、重伤时逐渐变黑的部分。
打开的文件的编辑模式也有SLP与SMX两种，其特性与文件相似，相当于合并数据和分离数据。
当打开SLD时，默认使用RGB555调色板（三个通道各32级，共32768色）与《帝国时代2：决定版》玩家色，玩家色会根据浓度和底色进行匹配。如果需要在其他版本的图像中使用，需要转换调色板。同时，由于SLD文件没有注释部分，因此打开后注释会变为形如“Alpha=255,Layer=1”的形式，您可以修改等号后面的值，分别对应图像不透明度（0~255）和所在图层（0为地面，1为立体）。在决定版更新后，DAT中的图层已经不再有用，图像所处的层次取决于SLD文件；实际上一个文件不同帧的图层可以不一样，但通常是相同的，因此这里没有进行区分。
...
您可以将部分或全部帧保存为GIF图像，其绘制的数据内容、玩家色和播放速率均取决于当前视图的状态，因此需在导出前调整。
...
您可以增加自定义的调色板，方法是将文件放在Palettes/Custom/文件夹下。支持的调色板包括游戏中的PAL、PALX格式（以文本形式描述颜色），以及Photoshop的ACT格式（以三个字节表示一种颜色）。这些调色板的编号会从513开始计算，但仅用于编辑，保存时请选择前面的调色板编号。
